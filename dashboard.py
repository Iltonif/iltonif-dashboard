...
.main { background: #05070f !important; }
...
with st.sidebar:
    ...
    <div style="padding:4px 0 24px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px">
        <div style="width:36px;height:36px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:9px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(99,102,241,0.4)">
        ...
        <div>
          <div style="font-size:1.1rem;font-weight:800;color:#f8faff;letter-spacing:-0.01em">ILTONIF</div>
          <div style="font-size:0.6rem;font-weight:600;letter-spacing:0.15em;color:#4b5280;text-transform:uppercase">Intelligence Platform</div>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown('<div style="font-size:0.65rem;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#4b5280;margin-bottom:10px">Filtros</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div style="height:1px;background:rgba(255,255,255,0.05);margin:16px 0"></div>', unsafe_allow_html=True)
    st.markdown('<div style="font-size:0.65rem;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#4b5280;margin-bottom:10px">Período</div>', unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="margin-top:12px;padding:12px 14px;background:#0d1120;border-radius:10px;border:1px solid rgba(255,255,255,0.05)">
      <div style="font-size:0.6rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#4b5280;margin-bottom:6px">Sistema</div>
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
        <div style="width:6px;height:6px;border-radius:50%;background:#10b981;box-shadow:0 0 6px #10b981"></div>
        <div style="font-size:0.72rem;color:#94a3b8">Pipeline activo</div>
      </div>
      <div style="font-size:0.68rem;color:#4b5280;font-family:'JetBrains Mono',monospace">{datetime.now().strftime('%d/%m/%Y %H:%M')}</div>
    </div>
    """, unsafe_allow_html=True)
...
with col_h1:
    ...
    <div style="padding:8px 0 24px">
      <div style="font-size:1.8rem;font-weight:800;color:#f8faff;letter-spacing:-0.02em;line-height:1.1">
        Panel de control
      </div>
      <div style="font-size:0.8rem;color:#4b5280;margin-top:4px;font-weight:400">
...
with col_h2:
    ...
    st.markdown(f"""
    <div style="padding:8px 0 24px;text-align:right">
      <div style="font-size:0.7rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#4b5280">Última actualización</div>
      <div style="font-size:0.95rem;font-weight:700;color:#818cf8;margin-top:2px;font-family:'JetBrains Mono',monospace">{today}</div>
    </div>
    """, unsafe_allow_html=True)
...
with c1: st.metric("🔴  Stock Crítico",  f"{criticos} SKUs",    delta=f"Acción urgente", delta_color="inverse")
...
with tab1:
        ...
        for _, row in df_sorted.iterrows():
            ...
            <div class="alert-card {css}">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
                <div>
                  <div style="font-weight:700;font-size:0.95rem;color:#f1f5f9;margin-bottom:3px">{row['Producto']}</div>
                  <div style="font-size:0.72rem;color:#4b5280">{row['Categoría']} &nbsp;·&nbsp; {row['Plataforma']}</div>
                </div>
                <div style="display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end">{badge_s}{badge_p}</div>
              </div>
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px">
                <div style="font-size:0.8rem;color:#64748b">📦 {row['accion_stock']}</div>
                <div style="font-size:0.8rem;color:#64748b">💰 {row['accion_pricing']}</div>
              </div>
              <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center">
                <span style="font-size:0.72rem;color:#334155">Precio: <b style="color:#94a3b8">{row['Precio actual']}€</b></span>
                <span style="font-size:0.72rem;color:#334155">Comp.mín: <b style="color:#94a3b8">{row['Comp. mín.']}€</b></span>
                <span style="font-size:0.72rem;color:#334155">Stock: <b style="color:#94a3b8">{row['Stock']} uds</b></span>
                <span style="font-size:0.72rem;color:#334155">Cobertura: <b style="color:#94a3b8">{row['Cobertura (días)']} días</b></span>
    ...
    with col_right:
        ...
        for label, val, color in [
            ("SKUs analizados", total_skus, "#818cf8"),
        ...
        st.markdown('<div class="section-title">Distribución stock</div>', unsafe_allow_html=True)

        COLORS_S = {"CRÍTICO":"#ef4444","REPOSICIÓN":"#f59e0b","EXCESO":"#6366f1","OK":"#10b981"}
        cs = df_rec["señal_stock"].value_counts().reset_index()
        cs.columns = ["Señal","N"]
        fig_donut = px.pie(cs, values="N", names="Señal", hole=0.72,
                           color="Señal", color_discrete_map=COLORS_S)
        fig_donut.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font_color="#64748b", height=220, margin=dict(t=0,b=0,l=0,r=0),
            showlegend=True,
            legend=dict(font=dict(color="#64748b",size=11), orientation="h", y=-0.1))
        fig_donut.update_traces(textfont_color="white", textfont_size=12)
        ...
        cp.columns = ["Señal","N"]
        fig_donut2 = px.pie(cp, values="N", names="Señal", hole=0.72,
        ...
        fig_donut2.update_layout(
            ...
            showlegend=True,
            legend=dict(font=dict(color="#64748b",size=11), orientation="h", y=-0.1))
        fig_donut2.update_traces(textfont_color="white", textfont_size=12)
    ...
    if sku_sel:
        ...
        for i, sku in enumerate(sku_sel):
            ...
            fig_line.add_trace(go.Scatter(
                ...
                fillcolor=f"rgba{tuple(list(bytes.fromhex(PALETTE[i%len(PALETTE)][1:])) + [15])}".replace("rgba(","rgba(").replace(",15)",",.06)") if i==0 else "rgba(0,0,0,0)"
            ))

        fig_line.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(13,17,32,0.5)",
            font_color="#64748b", height=380,
            legend=dict(orientation="h", y=-0.25, font=dict(color="#94a3b8", size=11)),
            xaxis=dict(gridcolor="rgba(255,255,255,0.04)", showline=False, tickfont=dict(size=11)),
            yaxis=dict(gridcolor="rgba(255,255,255,0.04)", showline=False, title="Unidades", tickfont=dict(size=11)),
            hovermode="x unified", margin=dict(t=10,b=10))
        st.plotly_chart(fig_line, use_container_width=True)

        col_h, col_e = st.columns(2, gap="large")
        with col_h:
            st.markdown('<div class="section-title">Estacionalidad por mes y día</div>', unsafe_allow_html=True)
            ...
            fig_h = px.imshow(pivot, color_continuous_scale=[[0,"#0d1120"],[0.5,"#4338ca"],[1,"#818cf8"]],
                              labels=dict(color="Uds/día"))
            fig_h.update_layout(paper_bgcolor="rgba(0,0,0,0)", font_color="#64748b", height=300,
...
with tab3:
    ...
    nombre_comp_map = {
        ...
        "precio_ortoweb": "Ortoweb",
        "precio_medicalexpo": "Medicalexpo",
        "precio_kayaks_es": "Kayaks.es",
    }

    PALETTE_P = ["#6366f1","#ef4444","#f59e0b","#10b981"]
    fig_bar = go.Figure()
    all_cols_p = ["precio_venta"] + comp_cols
    all_names_p = ["Tu precio"] + [nombre_comp_map.get(c, c.replace("precio_","").replace("_"," ").title()) for c in comp_cols]

    for i, (col, name) in enumerate(zip(all_cols_p, all_names_p)):
        fig_bar.add_trace(go.Bar(name=name, x=productos_list, y=ultimo_df[col],
                                 marker_color=PALETTE_P[i % len(PALETTE_P)],
                                 marker_line_width=0))
    fig_bar.update_layout(
        barmode="group", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(13,17,32,0.5)",
        font_color="#64748b", height=400, xaxis_tickangle=-30,
        legend=dict(orientation="h", y=-0.4, font=dict(color="#94a3b8")),
        xaxis=dict(gridcolor="rgba(255,255,255,0.03)", tickfont=dict(size=10)),
        yaxis=dict(gridcolor="rgba(255,255,255,0.03)", title="Precio (€)"),
    ...
    with col_ev:
        ...
        fig_ev = go.Figure()
        all_ev_cols = [("precio_venta","Tu precio","#6366f1","solid",2.5)] + \
        ...
        for col, name, color, dash, width in all_ev_cols:
        ...
        fig_ev.update_layout(
            ...
            legend=dict(orientation="h", y=-0.35, font=dict(color="#94a3b8",size=11)),
    ...
    with col_tabla:
        ...
        def color_pricing(val):
            m = {"PRECIO ALTO":"background-color:rgba(239,68,68,0.15);color:#ef4444",
                 "SUBIR PRECIO":"background-color:rgba(16,185,129,0.15);color:#10b981",
                 "ALERTA COMP.":"background-color:rgba(245,158,11,0.15);color:#f59e0b"}
            return m.get(val,"background-color:rgba(99,102,241,0.12);color:#818cf8")
...
with tab4:
    ...
    st.markdown('<div class="section-title">Tabla completa de alertas de stock</div>', unsafe_allow_html=True)
    ...
    def color_stock(val):
        m = {"CRÍTICO":"background-color:rgba(239,68,68,0.15);color:#ef4444",
             "REPOSICIÓN":"background-color:rgba(245,158,11,0.15);color:#f59e0b",
             "EXCESO":"background-color:rgba(99,102,241,0.15);color:#818cf8"}
        return m.get(val,"background-color:rgba(16,185,129,0.12);color:#10b981")
...
# ── FOOTER ────────────────────────────────────────────────────
st.markdown("""
<div style="text-align:center;padding:24px 0 8px;border-top:1px solid rgba(255,255,255,0.04);margin-top:16px">
  <span style="font-family:'JetBrains Mono',monospace;font-size:0.62rem;color:#1e293b;letter-spacing:0.15em">
